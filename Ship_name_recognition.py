...
if (1 == 1):
    ...
    '''返回值:           new_name: 反转后的文字                                            '''
    '''*************************************************************************************'''
    def reverse_name(old_name):
        new_name = old_name

        pattern = re.compile('[0-9]+')
        # findall是返回所有符合匹配的项，返回形式为数组
        match = pattern.findall(old_name)

        #### 判断原始图片文件名是否倒序
        # 1) 含有数字且最后一个字为汉字
        # 2）不含数字但第一个字开头为 '货' '港' '机' 等
        # 3) 满足以上两个条件且文件名（船名.后缀）总长度大于4

        # 文件名和图片中顺序一致，但和人类认知顺序相反
        possible_char = ['货', '港', '机']

        file_oldname = old_name.replace(" ", "")
        if len(file_oldname) > 0:
            if (match and '\u4e00' <= file_oldname[-1] <= '\u9fa5') or (file_oldname[0] in possible_char) and len(
                    file_oldname) > 1:
                # 0513货宿苏->苏宿货3150
                new_name = old_name[::-1]

            # 文件名和图片中顺序一致，并和人类认知顺序一致，需要将船名提到编号之前
            if (match and file_oldname[-1] in possible_char) and len(file_oldname) > 1:
                ship_name_cc = "".join(re.findall('[\u4e00-\u9fa5]', file_oldname))
                ship_name_dc = re.sub("\D", "", file_oldname)
                # 856阜阳港->阜阳港856
    ...
    def preProcess(Input_path, Output_path):
        ...
        if not os.listdir(output_path):
            ...
            for file in files:
                ...
                #cv_imwrite(output_path + file[:-4] + 'before_Process' + file[-4:], image)
        ...
        if not os.listdir(output_path):
            ...
            for file in files:  # 遍历文件夹
                if (file[-3:] == 'jpg'):
                    ...
                    # 倒序处理
                    new_name = reverse_name(file_oldname_list[-1][:-4]) + file_oldname_list[-1][-4:]
                    file_newname = str(file_count).zfill(6) + '_' + new_name.replace(" ", "")
                    print(file_newname)
                    im = im.convert('RGB')
